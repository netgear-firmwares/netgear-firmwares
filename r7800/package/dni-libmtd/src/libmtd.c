...
/*add black list for fix command injection issue*/
...
static int read_data(const char *file, void *buf, int buf_len)
{
	...
	fd = open(str_replace(file), O_RDONLY | O_CLOEXEC);
	if (fd == -1)
		return -1;
...
static int dev_get_major(struct libmtd *lib, int mtd_num, int *major, int *minor)
{
	...
	DNI_sprintf_s(file, sizeof(file), lib->mtd_dev, mtd_num);
...
static int dev_read_data(const char *patt, int mtd_num, void *buf, int buf_len)
{
	...
	DNI_sprintf_s(file, sizeof(file), patt, mtd_num);
...
static int type_str2int(const char *str)
{
	...
	len=DNI_sprintf_s(in_str, sizeof(in_str) - 1, str);
	if (!DNI_strcmp_s(in_str, sizeof(in_str), "nand"))
		return MTD_NANDFLASH;
	if (!DNI_strcmp_s(in_str, sizeof(in_str), "nor"))
		return MTD_NORFLASH;
	if (!DNI_strcmp_s(in_str, sizeof(in_str), "rom"))
		return MTD_ROM;
	if (!DNI_strcmp_s(in_str, sizeof(in_str), "absent"))
		return MTD_ABSENT;
	if (!DNI_strcmp_s(in_str, sizeof(in_str), "dataflash"))
		return MTD_DATAFLASH;
	if (!DNI_strcmp_s(in_str, sizeof(in_str), "ram"))
		return MTD_RAM;
	if (!DNI_strcmp_s(in_str, sizeof(in_str), "ubi"))
...
int mtd_dev_present(libmtd_t desc, int mtd_num) {
	...
	if (!lib->sysfs_supported)
		...
		DNI_sprintf_s(file, sizeof(file), lib->mtd, mtd_num);
		return !stat(file, &st);
	}
}
