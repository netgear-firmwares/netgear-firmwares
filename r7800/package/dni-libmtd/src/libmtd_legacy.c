...
static int proc_parse_next(struct proc_parse_info *pi)
{
	...
	DNI_memcpy_s(pi->name, len, p, len);
	pi->name[len] = '\0';
...
int legacy_get_dev_info(const char *node, struct mtd_dev_info *mtd)
{
	...
	fd = open(str_replace(node), O_RDONLY);
	if (fd == -1)
	...
	while (proc_parse_next(&pi)) {
		if (pi.mtd_num == mtd->mtd_num) {
			DNI_strcpy_s((char *)mtd->name, sizeof((char *)mtd->name), pi.name);
			return 0;
		}
	}
...
int legacy_get_dev_info1(int mtd_num, struct mtd_dev_info *mtd)
{
	...
	DNI_sprintf_s(node, sizeof(node), MTD_DEV_PATT, mtd_num);
