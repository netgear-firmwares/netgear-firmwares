...
FILE *dni_popen(const char *command, const char *type)
{
    if(!command || !type)
        return NULL;
...
int is_sda(char * dev)
{
	...
	while (fgets(line, sizeof(line), fp)) {
		if (DNI_sscanf_s(line, " %d %d %llu %15[^\n ]",
...
static char *get_device_vendor(char *dev)
{
	...
	for(i=0; i<2; i++){
		DNI_sprintf_s(path, sizeof(path), "/sys/block/%s/device/%s", dev, ven_mod[i]);
		if(!(fp = fopen(path, "r")))
			continue;
...
static char * get_usb_serial_num(char *part_name)
{
	...
	DNI_sprintf_s(file_name1, sizeof(file_name1), "/sys/block/%s/device/scsi_device/", disk_name_replace);
	DNI_sprintf_s(file_name2, sizeof(file_name2), "/tmp/%s_info_tmp", disk_name_replace);
	dni_safe_system(file_name2, NULL, R_NORMAL, "/bin/ls", file_name1, NULL);

	DNI_sprintf_s(path, sizeof(path), "/tmp/%s_info_tmp", disk_name);
	fp= fopen(path, "r");
	if (fp == NULL)
		goto ret;
	while (fgets(line, sizeof(line), fp)) {
	...
	if(bus_num != NULL){
		DNI_sprintf_s(path, sizeof(path), "/proc/scsi/usb-storage/%s", bus_num);
		fp= fopen(path, "r");
		if (fp == NULL)
			goto ret;
...
#define SD_CARD_UUID_FILE "/tmp/sd_card_uuid"
	...
	FILE *fp;
        ...
        memset(uuid,0,sizeof(uuid));
     	...
     	DNI_sprintf_s(disk_device, sizeof(disk_device), "/dev/%s", str_replace(part_name));
		dni_safe_system(SD_CARD_UUID_FILE, NULL, R_NORMAL, "/usr/sbin/vol_id", "-u", disk_device, NULL);
	...
	if (i == 0)
		return get_usb_serial_num(part_name);
	strncpy(uuid, buf, sizeof(uuid)-1);
...
static void get_device_id(struct disk_partition_info *disk)
{
	...
	if(strncmp(disk->name, SATA_DEV_NAME, 3) == 0)
		DNI_sprintf_s(id, sizeof(disk->device_id), "%s*%s", get_sata_serial_num(), disk->name + 3);
	else if(strncmp(disk->name, SD_CARD_DEV_NAME, 3) == 0)
		DNI_sprintf_s(id, sizeof(disk->device_id), "%s*%s", get_sd_card_serial_num(disk->name), disk->name + 3);
	else
		if ( atoi(disk->name + 3) > 0 ){
			DNI_sprintf_s(id, sizeof(disk->device_id), "%s*%s", get_usb_serial_num(disk->name), disk->name + 3);
		} else {
...
//			snprintf(id, sizeof(disk->device_id), "%s*0", get_usb_serial_num(disk->name));
			...
			DNI_sprintf_s(id, sizeof(disk->device_id), "%s*%s", get_usb_serial_num(disk->name), disk->name + 2);
		}
}
...
#define VOLUME_ID_FILE "/tmp/vol_id_1"
	...
	DNI_sprintf_s(disk_device, sizeof(disk_device), "/dev/%s",str_replace(part_name));
	dni_safe_system(VOLUME_ID_FILE, NULL, R_NORMAL, "/usr/sbin/vol_id", "-L", disk_device, NULL);
...
static void scan_disk_entries(struct list_head *head)
{
	...
	/*
	  ...
	  */
	while (fgets(line,sizeof(line),fp)) {
		if (DNI_sscanf_s(line, " %d %d %llu %127[^\n ]",
				&major, &minors, &capacity, part_name) != 4)
			continue;
		...
		if ((dfp = fopen(dfout, "r")) != NULL) {
			while (fgets(line, sizeof(line), dfp)) {
				if (DNI_sscanf_s(line, "%s %llu %*llu %*llu %*s %*s", path_name, &cap) != 2)
					continue;
