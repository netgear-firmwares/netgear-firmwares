...
#define CURRENT_TIME_PATH "device-location/syncTime"
	...
	int env = 0;
	p = config_get("x_agent_id"); // p = get_value_from_dto("xagentcfg[0].x_agent_id");
	...
	p = config_get("x_agent_claim_code"); // p = get_value_from_dto("xagentcfg[0].x_agent_claim_code");
	if(p != NULL)
...
static void setRaCloudTime()
{
	...
	if(data_flag == 1) {
		...
		system("echo '[ntpclient] setRaCloudTime, Query...' > /dev/console");
...
void primary_loop(int usd, int num_probes, int cycle_time)
{
	...
	/* as ntgr demand, reduce cycle time */
	cycle_time = 8;
...
int main(int argc, char *argv[]) {
	...
	if (argc == 1) {
		...
		if(strcmp(manual_ntp,"1") != 0){
			...
			strncpy(manual_ntp_server_tmp, hostname, sizeof(manual_ntp_server_tmp));
