...
void handle_special_char(char *src, char *des)
{
	char *p = src;
	char *q = des;
	char c;
...
void write_vie_lease(char* ip,char *mac,char *hostname)
{
  ...
  char cmd[512],ip_sec[256]={0}, mac_sec[256]={0}, hostname_sec[256]={0};
  handle_special_char(ip, ip_sec);
  handle_special_char(mac_align, mac_sec);
  handle_special_char(hostname, hostname_sec);
    sprintf (cmd,"/usr/share/udhcpd/write_vie_lease  '%s'  '%s'  '%s' &",ip_sec,mac_sec,hostname_sec);
    system (cmd);
