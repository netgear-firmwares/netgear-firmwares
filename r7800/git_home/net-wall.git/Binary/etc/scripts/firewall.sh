...
{
        local result=$(cat /tmp/netwall-result)
        local retrying

        if [ "$result" = "0" ]; then
                [ -f "/tmp/netwall-retrying" ] && retrying=$(cat /tmp/netwall-retrying)
                [ "$retrying" != "1" ] && /etc/scripts/firewall-retry.sh &
        else
                [ -f "/tmp/netwall-retry" ] && rm -rf /tmp/netwall-retry
                [ -f "/tmp/netwall-retrying" ] && rm -rf /tmp/netwall-retrying
        fi
}
