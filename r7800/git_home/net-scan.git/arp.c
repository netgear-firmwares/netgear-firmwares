...
void read_device_list(struct device_list *head)
{
	...
	p1 = head;
	for(j = 0; ; j++){
		snprintf(name, sizeof(name), "%s%d", "device_list",j);
		val = config_get(name);
		if(*val == '\0')
			break;
...
void remove_disconn_dhcp(struct in_addr ip)
{
	...
	while(fgets(line, sizeof(line), fp) != NULL) {
		result = sscanf(line, "%31s%255s", ipaddr,list_str);
		if (result == 2){
...
void show_arp_table(void)
{
	...
	fp = fopen(ARP_FILE, "w");
	if (fp == 0) {
		if(sb_fp) fclose(sb_fp);
		if(name_type_fp) fclose(name_type_fp);
		return;
	}
